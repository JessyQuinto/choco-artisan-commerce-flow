...
export const useResponsive = (breakpoints: Partial<BreakpointConfig> = {}) => {
  ...
  return {
    ...
    isDesktop,
    currentBreakpoint,
    isSmallScreen: windowSize.width < config.sm,
    isMediumScreen: windowSize.width >= config.md && windowSize.width < config.lg,
    isLargeScreen: windowSize.width >= config.lg,
    isXLScreen: windowSize.width >= config.xl,
    is2XLScreen: windowSize.width >= config['2xl'],
  };
};
